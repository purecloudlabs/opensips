...
static unsigned int route_flags;
	...
	if ((th_no_dlg_check_self_socket_tag(msg->rcv.bind_address) ||
		...
		for (i = 0; i < r_uri->u_params_no; i++) {
			if (r_uri->u_name[i].len == th_contact_encode_param.len &&
				...
				if (th_no_dlg_seq_handling(msg, &r_uri->u_val[i])) {
					return !(route_flags & ROUTE_LOOSE) ? TOPOH_MATCH_SUCCESS : TOPOH_MATCH_LOOSE_ROUTE;
				}
...
static inline int topo_no_dlg_route(struct sip_msg *msg, str rr_buf[static 1], struct lump lmp[static 1]) {
	...
	int size = 0, start_index;
	int route_classification;
	...
	route_classification = topo_no_dlg_classify_route(head);
	...
	if (route_classification & ROUTE_LOOSE) {
		...
		if ((route_classification & ROUTE_DOUBLE_RR) && (route_classification & ROUTE_SELF)) {
			...
			if (rrp == NULL) {
				LM_ERR("Second Route header not set when r2=on and is self\n");
				route_flags = ROUTE_FAILURE;
				goto cleanup;
			}

			route_classification = topo_no_dlg_classify_route(rrp);
		...
		route_flags |= route_classification;
	} else {
		...
		if (rrp != NULL && start_index < rr_buf->len) {
			...
			rr_buf->len = rr_buf->len - head->len - 1;
			rr_buf->s = memcpy(rr_buf->s, route, rr_buf->len);
		}
...
cleanup:
	...
	return (route_flags & ROUTE_FAILURE) ? -1 : 1;
}
...
static int th_no_dlg_seq_handling(struct sip_msg *msg, str *info) {
	int max_size, size, port, proto;
	char *p = NULL, *route = NULL, *decode_buffer = NULL, *msg_buf = NULL;
	str rr_buf, ct_buf, flags_buf, bind_buf, host, route_buf = STR_NULL;
	struct hdr_field *it;
	...
	if (rr_buf.len) {
		if (topo_no_dlg_route(msg, &rr_buf, lmp) != 1) {
			LM_ERR("Failure to Route\n");
			goto err_fail_early;
		}
	}

	if ((route_flags & ROUTE_SUCCESS) && !(route_flags & ROUTE_STRICT) && ct_buf.len && ct_buf.s) {
	...
	} else if ((route_flags & ROUTE_SUCCESS) && (route_flags & ROUTE_STRICT) && ct_buf.len && ct_buf.s) {
