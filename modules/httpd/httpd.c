...
static int set_auth_realm(modparam_t t, void *v)
{ struct httpd_server *s; str r; s = httpd_srv_str(v, &r); if (!s) return -1; s->auth_realm = r; return 0; }

static int set_auth_username(modparam_t t, void *v)
{ struct httpd_server *s; str r; s = httpd_srv_str(v, &r); if (!s) return -1; s->auth_username = r; return 0; }

static int set_auth_password(modparam_t t, void *v)
{ struct httpd_server *s; str r; s = httpd_srv_str(v, &r); if (!s) return -1; s->auth_password = r; return 0; }
...
static const param_export_t params[] = {
	...
	{"auth_realm",       STR_PARAM|USE_FUNC_PARAM,           (void*)set_auth_realm},
	{"auth_username",    STR_PARAM|USE_FUNC_PARAM,           (void*)set_auth_username},
	{"auth_password",    STR_PARAM|USE_FUNC_PARAM,           (void*)set_auth_password},
...
static int mod_init(void)
{
	...
	for (i = 0; i < httpd_n_servers; i++) {
		...
		if (s->auth_realm.s)
			s->auth_realm.len = strlen(s->auth_realm.s);
		if (s->auth_username.s)
			s->auth_username.len = strlen(s->auth_username.s);
		if (s->auth_password.s)
			s->auth_password.len = strlen(s->auth_password.s);

		if ((s->auth_username.s && !s->auth_password.s) ||
				(!s->auth_username.s && s->auth_password.s)) {
			LM_ERR("both auth_username and auth_password must be set "
				"(HTTP server '%.*s')\n", s->name.len, s->name.s);
			return -1;
		}
		if (s->auth_username.s && s->auth_username.len == 0) {
		...
		if (s->post_buf_size < MIN_POST_BUF_SIZE) {
			LM_ERR("post_buf_size should be bigger than %d "
				"(HTTP server '%.*s')\n",
