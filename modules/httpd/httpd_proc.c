...
void httpd_proc(int rank)
{
	...
	/* Map this worker rank to its server instance */
	for (i = 0; i < httpd_n_servers; i++) {
		o = &httpd_servers[i];
	...
	/* Set per-server globals for this worker */
	ip = s->ip;
	port = s->port;
